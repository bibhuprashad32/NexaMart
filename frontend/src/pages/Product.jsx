...
const Product = () => {
  ...
  const retrieveProduct = async () => {
    const response = await axios.get(
      "http://localhost:9090/api/product/id?productId=" + productId
    );
  ...
  const retrieveProductsByCategory = async () => {
    const response = await axios.get(
      "http://localhost:9090/api/product/category?categoryId=" + categoryId
    );
  ...
  const saveProductToCart = (userId) => {
    fetch("http://localhost:9090/api/user/cart/add", {
      method: "POST",
      headers: {
      ...
      <div class="row">
        ...
        <div class="col-sm-3 mt-2 admin">
          <div class="card form-card border-color custom-bg">
            <img
              src={"http://localhost:9090/api/product/" + product.imageName}
              style={{
                maxHeight: "500px",
                maxWidth: "100%",
                width: "auto",
